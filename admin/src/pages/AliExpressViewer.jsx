...
console.log(data);
      const skuData = data?.aliexpress_ds_product_get_response?.result?.ae_item_sku_info_dtos?.ae_item_sku_info_d_t_o || [];
  ...
  return (
    <div className="bg-gradient-to-br border rounded border-gray-200 min-h-screen flex items-center justify-center">
      <div className="w-full bg-white p-8 rounded-2xl">
        <h1 className="text-2xl font-bold text-center text-blue-700 mb-6">AliExpress Product Viewer</h1>

        <form onSubmit={fetchProductInfo} className="space-y-4 mb-8 w-96 mx-auto">
          <label htmlFor="product_id" className="block text-gray-700 font-semibold">Enter Product ID</label>
          <input
            type="text"
            id="product_id"
            value={productId}
        ...
        {skus.length === 0 && !error && !loading && (
          <p className="text-yellow-600 font-medium text-center">No SKU data found.</p>
        )}
 
        <div className="space-y-6">
          {skus.map((sku, index) => (
            <div key={index} className="border border-gray-200 p-4 rounded-lg shadow-sm bg-gray-50">
              <p><strong>SKU Attribute:</strong> {sku.sku_attr}</p>
              <p><strong>Sale Price:</strong> {sku.offer_sale_price} {sku.currency_code}</p>
              <p><strong>Stock:</strong> {sku.sku_available_stock}</p>
              <p><strong>SKU ID:</strong> {sku.sku_id}</p>
              <p><strong>SKU Code:</strong> {sku.sku_code}</p>

              {(sku.ae_sku_property_dtos?.ae_sku_property_d_t_o || []).map((prop, i) => (
                <div key={i} className="mt-2 pl-2 border-l-4 border-blue-500">
                  <p><strong>Property Name:</strong> {prop.sku_property_name}</p>
                  <p><strong>Property Value:</strong> {prop.sku_property_value}</p>
                  {prop.sku_image && (
                    <img
                      src={prop.sku_image}
                      alt="SKU"
